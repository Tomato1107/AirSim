...
/**
 * @brief Parameter specialization for MIT Aerospace Controls Lab Snap Stack autopilot
 * @author Parker Lusk <plusk@mit.edu>
 * @date 18 March 2020
 */

#ifndef msr_airlib_vehicles_SnapStackParams_hpp
#define msr_airlib_vehicles_SnapStackParams_hpp

#include "common/AirSimSettings.hpp"
#include "vehicles/multirotor/MultiRotorParams.hpp"
#include "vehicles/multirotor/firmwares/snapstack/SnapStackApi.hpp"
#include "sensors/SensorFactory.hpp"


namespace msr { namespace airlib {

class SnapStackParams : public MultiRotorParams {
public:
    SnapStackParams(const AirSimSettings::VehicleSetting* vehicle_setting, std::shared_ptr<const SensorFactory> sensor_factory)
        : vehicle_setting_(vehicle_setting), sensor_factory_(sensor_factory)
    {
        // connection_info_ = getConnectionInfo(vehicle_setting);
    }

    virtual ~SnapStackParams() = default;

    virtual std::unique_ptr<MultirotorApiBase> createMultirotorApi() override
    {
        return std::unique_ptr<MultirotorApiBase>(new SnapStackApi(this, vehicle_setting_));
    }
